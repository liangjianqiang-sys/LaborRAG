...
> 🖼️ **截图待填充**：启动系统后截 3 张图保存到 `docs/screenshots/`，然后取消下方注释即可。

<!-- 截图后取消下面整段注释：
| 问答页 | 评估结果 | 知识库管理 |
|:---:|:---:|:---:|
| ![问答](docs/screenshots/chat.png) | ![评估](docs/screenshots/eval.png) | ![知识库](docs/screenshots/kb.png) |
| *智能问答 + 来源追溯* | *三元组评估指标* | *法条结构化切分* |
-->

<details>
<summary><b>📋 截图指南（点击展开）</b></summary>

启动后端 + 前端后，截以下 3 张图保存到 `docs/screenshots/` 目录：

| 文件名 | 页面 | 截什么 |
|--------|------|--------|
| `chat.png` | 问答页 | 一次完整的法条查询对话（问题 + 回答 + 来源引用展开），证明系统可交互 |
| `eval.png` | 评估结果页 | 20 题评估指标汇总（P@1=90%、R@5=92.5%、幻觉率=8%），证明有量化评估 |
| `kb.png` | 知识库管理页 | 17 部法律文档列表 + chunks 统计（1243），证明数据处理能力 |
