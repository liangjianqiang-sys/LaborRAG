...
from app.utils.law_refs import _SOURCE_LAW_MAP, normalize_article  # noqa: F401
# 注：normalize_article / _SOURCE_LAW_MAP 的唯一真相源都在 app/utils/law_refs。
# 此处再导出，保留既有调用方 `from app.utils.text import normalize_article`。
...
def _law_display(source: str) -> str:
    ...
    映射表从 app/utils/law_refs 取（最底层），而不是从 retrieval 层 ——
    后者会让 utils 反向依赖上层，构成包级循环。
    """
