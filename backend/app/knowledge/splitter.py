...
from app.utils.law_refs import ARTICLE_REF_PATTERN
...
def _new_recursive_splitter():
    ...
    路径需要它。放在模块级会让所有 import app.knowledge.splitter 的代码
...
class LawArticleSplitter:
    ...
    # 匹配 "第X条" 的正则。锚定行首 + 后随空白，语义与 law_refs 中的
    # 通用模式不同，故在此处组合而非直接复用 ARTICLE_REF_RE
    ARTICLE_PATTERN = re.compile(rf"^{ARTICLE_REF_PATTERN}\s")
