...
from app.evaluation.utils import parallel_map


from app.evaluation import persistent_store as store
# 以下名字在本模块内继续可用（函数在调用时解析各自模块的全局，故别名安全）；
# `load_all_tasks` / `save_task` 另有外部引用（app/main.py）。
from app.evaluation.persistent_store import (  # noqa: F401
    load_all_tasks,
    save_task,
    _ensure_dirs,
    _load_task,
...
class PersistentEvalManager:
    ...
    def list_tasks(self) -> List[Dict[str, Any]]:
        ...
        _ensure_dirs()
        tasks = []
        for filename in os.listdir(store._TASKS_DIR):
    ...
    def _run_ragas_scoring(
        ...
        from app.evaluation.runner import _mean_scores
